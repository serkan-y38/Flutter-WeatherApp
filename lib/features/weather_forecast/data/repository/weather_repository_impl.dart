...
class WeatherRepositoryImpl implements WeatherRepository {
  ...
  @override
  Future<Resource<WeatherForecastEntity>> getWeatherForecast(
      String city) async {
    try {
      Loading;
      final res = await _weatherApiService.getWeatherForecast(city: city);
      if (res.response.statusCode == HttpStatus.ok) {
        var data = res.data.weatherForecastResponseToDomain();
        return Success(data);
      } else {
        return Error(DioException(
            error: res.response.statusCode,
